...
def xgb_results(X, y, X_train, X_test, y_train, y_test,):
    #Hyperparameter tuning
...
def ebm_fit(X, y, X_train, X_test, y_train, y_test):
    random_search = ebm_tuning_cv(X,y)
    ebm = ExplainableBoostingClassifier(**random_search.best_params_)
    emb_fit = ebm.fit(X_train, y_train)
    score = ebm.score(X_test, y_test)
    auc = roc_auc_score(y_test, emb_fit.predict_proba(X_test)[:, 1])
